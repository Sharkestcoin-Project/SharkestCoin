...
#if defined(NDEBUG)
# error "Sharkestcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x4b65454be5975819b1a284c07a68bc37935b361902c806673078352353d9216c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Sharkestcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Sharkestcoin Signed Message:\n";
...
{
    // Sharkestcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Sharkestcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Sharkestcoin: 3 days
static const int64 nTargetSpacing = 2 * 60; // Sharkestcoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Sharkestcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Sharkestcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x34ba59fe038c48da9a7ce81bb0454a079b66248c3d6ba081a5f8fe568a049282");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "18:12/ 17/03/2021, Corona Virus Pandemic outbreak in school";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 250 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04426317bcc1b3c7a3d427042b0c38637bd47000878aa2042c4a60979cbcb11eb9b9ce2339f0b79438f459e7e7fec0617c7184c594c152ef6849691aef057bad6a") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1616004633;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084633036;

        if (fTestNet)
        {
            block.nTime    = 1616004569;
            block.nNonce   = 385947773;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x0c7e8622e9ae7c27d0050ed36ce481045dce698e502b904aa614e2c0c8571e68"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc3, 0xb2, 0xde }; // Sharkestcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SharkestcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SharkestcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SharkestcoinMiner(CWallet *pwallet)
{
    printf("SharkestcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sharkestcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SharkestcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SharkestcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SharkestcoinMiner, pwallet));
}
